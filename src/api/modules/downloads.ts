...
import { toDownloadGroups, toQuickFiles } from '@/api/sheet-mappers'
...
export const fetchDownloadGroups = async (): Promise<DownloadGroup[]> =>
  hasBackend()
    ...
    : fromSheet('downloads', toDownloadGroups, async () =>
...
export const fetchQuickFiles = async (): Promise<DownloadFile[]> =>
  hasBackend()
    ...
    : fromSheet('quick', toQuickFiles, async () => fromStatic((await local()).quickFilesData))
